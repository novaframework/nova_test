...
path() ->
    ?LET(Segments, non_empty(list(path_segment())),
         list_to_binary(["/" | lists:join("/", Segments)])).

path_segment() ->
    ?LET(S, non_empty(list(oneof([range($a, $z), range($0, $9), $-]))),
         list_to_binary(S)).

header_name() ->
    ?LET(Parts, non_empty(list(header_word())),
...
json_text() ->
    ?LET(S, non_empty(list(oneof([range($a, $z), range($A, $Z), range($0, $9), $_, $-]))),
         list_to_binary(S)).
...
prop_new_has_required_keys() ->
    ?FORALL({Method, Path}, {http_method(), path()},
        begin
            Req = nova_test_req:new(Method, Path),
            lists:all(fun(K) -> maps:is_key(K, Req) end,
...
prop_headers_accumulate() ->
    ?FORALL({N1, V1, N2, V2}, {header_name(), header_value(), header_name(), header_value()},
        ?IMPLIES(N1 =/= N2,
            begin
                Req = nova_test_req:with_header(N2, V2,
                      nova_test_req:with_header(N1, V1,
...
prop_header_last_write_wins() ->
    ?FORALL({Name, V1, V2}, {header_name(), header_value(), header_value()},
        begin
            Req = nova_test_req:with_header(Name, V2,
                  nova_test_req:with_header(Name, V1,
                  nova_test_req:new(get, <<"/t">>))),
...
prop_bindings_exact() ->
    ?FORALL(Bindings, map(query_key(), query_value()),
        begin
...
prop_query_contains_all_pairs() ->
    ?FORALL(Params, non_empty_query_map(),
        begin
...
prop_peer_roundtrip() ->
    ?FORALL({A, B, C, D, Port}, {ip_byte(), ip_byte(), ip_byte(), ip_byte(), port_number()},
        begin
            Peer = {{A, B, C, D}, Port},
...
prop_auth_data_roundtrip() ->
    ?FORALL(AuthData, map(atom(), binary()),
        begin
...
prop_json_sets_content_type() ->
    ?FORALL(Json, map(json_text(), json_text()),
        begin
